...
function luoTilaus($henkilo_id, $tapahtuma_id) {
    DB::run(
        'INSERT INTO tilaukset (henkilo_id, tapahtuma_id) VALUES (?,?)',
...
function lisaaTilausRivi($tilaus_id, $tapahtuma_id, $maara) {
    DB::run(
        'INSERT INTO tilausrivit (tilaus_id, tapahtuma_id, maara) VALUES (?,?,?)',
...
function paivitaVarasto($maara, $idtapahtuma) {
    DB::run(
        'UPDATE tapahtumat SET varasto = varasto - ?
         WHERE idtapahtuma = ?',
        [$maara, $idtapahtuma]
    );
}
...
function varastoaJaljella($tapahtuma_id) {
    $tulos = DB::run(
        'SELECT varasto / alkuvarasto AS osuus FROM tapahtumat 
         WHERE idtapahtuma = ?',
        [$tapahtuma_id]
    )->fetch();
...
function varastotilanne($tapahtuma_id) {
    $tulos = DB::run('SELECT varasto FROM tapahtumat 
         WHERE idtapahtuma = ?',
        [$tapahtuma_id]
    )->fetch();
