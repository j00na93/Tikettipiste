...
function lisaaTili($formdata, $baseurl='') {
  ...
  // Tarkistetaan, että puhelinnumero on määritelty

  if (!isset($formdata['puh']) || !$formdata['puh']) {
    $error['puh'] = "Anna puhelinnumerosi";
  } else {
if (!preg_match("/^(?:\+358|0)\s?\d[\d\s-]{4,12}$/", $formdata['puh'])) {
    $error['puh'] = "Puhelinnumero on virheellinen.";
  }
  }
  ...
  if (!$error) {
    ...
    $idhenkilo = lisaaHenkilo($nimi,$email,$puh,$salasana);
...
function lahetaVahvavain($email,$url) {
  $message = "Hei!\n\n" . 
             "Olet rekisteröitynyt Tikettipiste-palveluun tällä\n" . 
             "sähköpostiosoitteella. Klikkaamalla alla olevaa\n" . 
             "linkkiä vahvistat käyttämäsi sähköpostiosoitteen\n" .
             "ja pääset käyttämään Tikettipiste-palvelua.\n\n" . 
             "$url\n\n" .
             "Jos et ole rekisteröitynyt Tikettipiste palveluun, niin\n" . 
             "silloin tämä sähköposti on tullut sinulle\n" .
             "vahingossa. Siinä tapauksessa ole hyvä ja\n" .
             "poista tämä viesti.\n\n".
             "Terveisin, Tikettipiste-palvelu";
  return mail($email,'Tikettipiste-tilin aktivointilinkki',$message);
}
...
function lahetaVaihtoavain($email,$url) {
  $message = "Hei!\n\n" .
             ...
             "Terveisin, Tikettipiste-palvelu";
  return mail($email,'Tikettipiste-tilin salasanan vaihtaminen',$message);
}
